...
%% helper function
function J_N = terminal_cost(T1,T2,alpha,beta,dim_flag)
    if strcmp(dim_flag,'1D')
        J_N = reshape(state_cost(T1,alpha,beta,dim_flag), numel(T1), 1);
    elseif strcmp(dim_flag,'2D')
        J_N = reshape(state_cost(cat(3,T1,T2),alpha,beta,dim_flag), numel(T1), 1);
    else
